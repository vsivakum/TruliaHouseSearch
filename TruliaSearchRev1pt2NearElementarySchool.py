...
def uniq(input):
  ...
  return output 
def TruliaSearch():
  output_file = codecs.open("C:\Python27\outlist1pt2elemhi.txt",'w','utf-8')
  station_list = []  
  trimet_lat_list = []
  trimet_long_list = []
  ...
  numunknown=0
  numtotal=0
  for k in range(0,len(elemschool_list)): 
    start_price = min_price
    reflat=elem_lat_list[k]
    reflong=elem_long_list[k]
    elemschool=elemschool_list[k]
    min_trimet_lat=float(reflat)-1.8/69
    min_trimet_long=float(reflong)-1.8/52
    max_trimet_lat=float(reflat)+1.8/69
    max_trimet_long=float(reflong)+1.8/52  
    for j in range(0,20):
      ...
      print "Elementary:"+str(elemschool)
      if str(Numresults[0])=='\n        0 Results      ':continue	  
      for i in range(0,len(urls)):
        ...
        for m in range(0,len(station_list)):
          trimet_lat=trimet_lat_list[m]
          trimet_long=trimet_long_list[m]  
          station = station_list[m]
          dist = math.sqrt(math.pow(math.fabs(float(houselat)-float(trimet_lat))*69,2)+math.pow(math.fabs(float(houselong)-float(trimet_long))*52,2))
          print "Distance from "+str(station)+" MAX:"+str(dist)+" miles"
